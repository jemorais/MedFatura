...
var app = new Hono2();
app.use("*", cors({
  origin: ["http://localhost:5173", "https://medfatura.com"],
  credentials: true,
...
var authMiddleware = /* @__PURE__ */ __name(async (c, next) => {
  ...
  if (authHeader && authHeader.startsWith("Bearer ")) {
    ...
    const supabase2 = createSupabaseClient(c);
    try {
      const { data: { user }, error } = await supabase2.auth.getUser(token);
      if (error || !user) {
        return c.json({ error: "Token inv\xE1lido" }, 401);
      }
      const { data: userData, error: userError } = await supabase2.from("users").select("id, email, name, user_type, cpf_crm").eq("email", user.email).single();
      ...
      return c.json({ error: "Erro de autentica\xE7\xE3o" }, 401);
    }
  }
  ...
  try {
    const { data: session, error: sessionError } = await supabase.from("sessions").select("user_id").eq("id", sessionId).gt("expires_at", (/* @__PURE__ */ new Date()).toISOString()).single();
    if (sessionError || !session) {
      return c.json({ error: "Sess\xE3o inv\xE1lida ou expirada" }, 401);
    }
    const { data: user, error: userError } = await supabase.from("users").select("id, email, name, user_type, cpf_crm").eq("id", session.user_id).single();
    if (userError || !user) {
      return c.json({ error: "Usu\xE1rio n\xE3o encontrado" }, 401);
    }
    c.set("user", user);
...
app.post("/api/auth/login", zValidator("json", external_exports.object({
  ...
  try {
    ...
    const { data: userData, error: userError } = await supabase.from("users").select("id, email, name, user_type, cpf_crm").eq("email", email).single();
    if (userError || !userData) {
      return c.json({ error: "Usu\xE1rio n\xE3o encontrado" }, 401);
    }
    ...
    if (sessionError) {
      return c.json({ error: "Erro ao criar sess\xE3o" }, 500);
    }
...
app.get("/api/invoices", authMiddleware, async (c) => {
  ...
  const supabase = createSupabaseClient(c);
  try {
    let query = supabase.from("invoices").select("*").eq("deleted_by_user", false).eq("deleted_by_admin", false).order("created_at", { ascending: false });
    if (user.user_type !== "admin") {
      query = query.eq("user_id", user.id);
    }
    const { data: invoices, error } = await query;
    if (error) {
      return c.json({ error: "Erro ao buscar faturas" }, 500);
    }
...
app.post("/api/invoices/upload", authMiddleware, async (c) => {
  ...
  try {
    ...
    const storedFilename = `invoice_${user.id}_${timestamp}.pdf`;
    const fileBuffer = await file.arrayBuffer();
    const { error: uploadError } = await supabase.storage.from("invoices").upload(storedFilename, fileBuffer, {
    ...
    if (dbError) {
      await supabase.storage.from("invoices").remove([storedFilename]);
      return c.json({ error: "Erro ao salvar dados da fatura" }, 500);
    }
    return c.json({
...
app.get("/api/invoices/:id/download", authMiddleware, async (c) => {
  ...
  const supabase = createSupabaseClient(c);
  try {
    let query = supabase.from("invoices").select("*").eq("id", invoiceId).eq("deleted_by_user", false).eq("deleted_by_admin", false);
    if (user.user_type !== "admin") {
      query = query.eq("user_id", user.id);
    }
    const { data: invoice, error } = await query.single();
    if (error || !invoice) {
      return c.json({ error: "Fatura n\xE3o encontrada" }, 404);
    }
    const { data: fileData, error: downloadError } = await supabase.storage.from("invoices").download(invoice.stored_filename);
    ...
    return new Response(arrayBuffer, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${invoice.original_filename}"`,
...
app.get("/api/pending-payments", authMiddleware, adminMiddleware, async (c) => {
  const supabase = createSupabaseClient(c);
  try {
    const { data: payments, error } = await supabase.from("pending_payments").select(`
        *,
        user:users!pending_payments_user_id_fkey(id, name, email)
      `).order("created_at", { ascending: false });
    if (error) {
      return c.json({ error: "Erro ao buscar pagamentos pendentes" }, 500);
    }
...
app.post("/api/pending-payments", authMiddleware, adminMiddleware, zValidator("json", external_exports.object({
  ...
  const user = c.get("user");
  const { user_id, amount, description, month, year } = c.req.valid("json");
  const supabase = createSupabaseClient(c);
  try {
    ...
    if (error) {
      return c.json({ error: "Erro ao criar pagamento pendente" }, 500);
    }
    return c.json({
...
app.get("/api/notifications", authMiddleware, async (c) => {
  ...
  try {
    ...
    if (error) {
      return c.json({ error: "Erro ao buscar notifica\xE7\xF5es" }, 500);
    }
...
app.put("/api/notifications/:id/read", authMiddleware, async (c) => {
  ...
  try {
    ...
    if (error) {
      return c.json({ error: "Erro ao marcar notifica\xE7\xE3o como lida" }, 500);
    }
    return c.json({ message: "Notifica\xE7\xE3o marcada como lida" });
  } catch (error) {
    return c.json({ error: "Erro interno do servidor" }, 500);
  }
});
app.get("/api/stats", authMiddleware, adminMiddleware, async (c) => {
  const supabase = createSupabaseClient(c);
  try {
    const [usersResult, invoicesResult, paymentsResult] = await Promise.all([
      supabase.from("users").select("id", { count: "exact" }),
      supabase.from("invoices").select("id", { count: "exact" }).eq("deleted_by_user", false).eq("deleted_by_admin", false),
      supabase.from("pending_payments").select("id", { count: "exact" }).eq("status", "pending")
    ]);
    return c.json({
      stats: {
        total_users: usersResult.count || 0,
        total_invoices: invoicesResult.count || 0,
        pending_payments: paymentsResult.count || 0
      }
    });
  } catch (error) {
    return c.json({ error: "Erro interno do servidor" }, 500);
